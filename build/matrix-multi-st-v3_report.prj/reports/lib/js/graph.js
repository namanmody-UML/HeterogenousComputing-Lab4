...
function StartGraph(mavData, graphConstObj, kernelName, lmemName, bankList, bankName, replFlag) {
        ...
        var tt = function (n) {
            ...
            Object.keys(details).filter(function(d) {
                ...
                if (!(hiddenDetails[k]) && !(k.indexOf("Arguments from") != -1 && k.indexOf(kernelName) == -1)) {
                    let html_tmp = details[k];
                    if (k === "Name" || k === "Pipe Name") html_tmp = html_tmp.replace(/</g, '&lt;').replace(/>/g, '&gt;')
                    text += k + ": " + html_tmp + "<br>";
                }
            });
            text += "</p>";
    ...
    function setupGraph() {
        ...
        var offsetX = (panelWidth - graphWidth * zoomFitScale) / 2; // Offset to center the graph in the middle
        var offsetY = 0; // The best appearance is actually to have top of graph at top of panel
        ...
        // Place the graph in centre
        // MOD: ddoering - there was an issue where the canvas was being created based on the flat graph size,
        // not the scaled graph size so the canvas was way outside of the card panel.
        // Fixed by scaling graph when determining SVG size.
        spgSVG.attr("height", panelHeight);
        spgSVG.attr("width", panelWidth);
    ...
    function createNodes(group, nodes, lmemName, bankList) {
        ...
        function createCopiesNode(num_copies, copies_name, parent_name, copies_details, padding_size, depth_size) {
            ...
            if( !isIE && !isEdge){
                ...
                spg.setNode(copies_name, {
                  label: function() {
                    ...
                    var hasPadding = (padding_size > 0) ? true : false;
                    var i, j, row;
                    ...
                    var initialRatio = (depth_size - padding_size) / depth_size;
                    var paddingRatio = (initialRatio < 0.8) ? (initialRatio) : 0.8;
                    var heightPerCopy = (num_copies > 4) ? (100/4.25) : (100/num_copies);
                    var numIterations = (hasPadding) ? (max_visible_copies * 2) : max_visible_copies;
                    ...
                    if(num_copies > 4){
                        var numAdditionalRows = (hasPadding) ? 3 : 2;
                        for (j = 0; j < numAdditionalRows; j++,i++){
                            ...
                            if(j === 0) {
                                ...
                                row.style.height = (heightPerCopy * paddingRatio) + "%";
                                row.innerHTML = "Copy " + (num_copies - 1);
                            } else {
                                row.innerHTML = "Padding " + (num_copies - 1);
                                row.style.height = (heightPerCopy * (1 - paddingRatio)) + "%";
                                row.style.backgroundColor = "lightgrey";
                            }
                            row.style.width = "100%";
            ...
            if (n.children) {
                ...
                if(replFlag && n.type == "bank" && n.name == bankName) {
                    ...
                    if(typeof n.copies !== "undefined") {
                        ...
                        if(num_copies > 0) {
                            createCopiesNode(num_copies, copies_name, n.name, n.copies.details, n.padding, n.depth);
    ...
    this.zoomToFit = function (duration) {
        ...
        var offsetX = (panelWidth - graphWidth * zoomFitScale)/2; // Offset to center the graph in the middle
        var offsetY = 0; // Offset nothing vertically
