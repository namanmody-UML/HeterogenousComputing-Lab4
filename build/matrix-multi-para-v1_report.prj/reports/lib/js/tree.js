      ...
      click: function (event, data) {
        ...
        if (curElemID === getTreeIDName(VIEWS.GV.id)) {
          ...
          if (pipelineJSON[nodeId] !== undefined) {
            ...
            let gv_graph = new FPGAGVGraph(pipelineJSON[nodeId]);
          } else if (blockJSON[nodeId] !== undefined) {
            $("#GVG").parent().find(".currentEntity").text(": " + data.node.title);
            let gv_graph = new FPGAGVGraph(blockJSON[nodeId]);
            ...
            if (product == PRODUCTS.HLS) {
              ...
              if (isValidSystemViewer) {
                ...
                setTimeout(function() {
                  let cspv_graph = new FPGAGVGraph(mavJSON);
                }, 20);
              } // end backward compitable
            } else {
              // OpenCL: no function viewer yet this will create a message about no graph
              let opencl_graph = new FPGAGVGraph(undefined);
            }
          }
        }
          ...
          if (data.node.data.type === "memsys" || data.node.data.type === "romsys") {
            ...
            let lmv_graph = new FPGALMVGraph(new_lmvJSON, lmem_name, bank_name);
