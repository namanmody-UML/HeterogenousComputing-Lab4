...
function preProcessNodes() {
  ...
  for (let i = 0; i < loopDataNodes.length; i++) {
    ...
    if (loopSpeculationDict.hasOwnProperty(loopNode.name)) {
      ...
      if (! isNaN(speculation)) { loopNode['speculation'] = speculation; }
    }

    if (loopNode.hasOwnProperty('ii')) loopNode['ii'] = parseInt(loopNode['ii']);  // II
    if (loopNode.hasOwnProperty('ll')) loopNode['ll']= parseInt(loopNode['ll']);  // loop layer
    if (loopNode.hasOwnProperty('pl')) loopNode['pl'] = (loopNode['pl'] === 'Yes') ? 1 : 0;  // pipelined
